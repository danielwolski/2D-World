...
World::World(World&& other)
{
	...
	this->czyJestKoniecGry = other.czyJestKoniecGry;
	this->organisms = other.organisms;
	this->czlowiek = other.czlowiek;
	...
	other.czyJestKoniecGry = false;
	other.czlowiek = nullptr;
}
...
void World::generate_world() {
	Point position = WylosujWolnePole();
	...
	for (int i = 0; i < NUMBER_OF_ANIMALS_AT_THEBEG; i++)
	{
		...
		position = WylosujWolnePole();
		if (!(position == Point(-1, -1))) {
			add_organism(organisms_maker::create_new_organism(Organism::OrganismType::BARSZCZ_SOSNOWSKIEGO, this, position));
		}
		else return;
	}
}
...
void World::makeTurn()
{
	...
	for (size_t i = 0; i < organisms.size(); i++) {
		...
		else if (organisms[i]->GetTuraUrodzenia() != turn_number) {
			organisms[i]->action();
			drawWorld();
		}
	}
...
bool World::GetCzyCzlowiekZyje()
{
	return is_player_alive;
}

void World::SetCzyCzlowiekZyje(bool is_player_alive)
{
...
void World::SetCzyJestKoniecGry(bool czyJestKoniecGry)
{
	...
	while (true) {
		...
		if (input == IF_ARROWS) {
			if (is_player_alive) {
				input = _getch();
				if (input == UP) {
					czlowiek->set_direction(Organism::Direction::GORA);
					break;
				}
				else if (input == DOWN) {
					czlowiek->set_direction(Organism::Direction::DOL);
					break;
				}
				else if (input == LEFT) {
					czlowiek->set_direction(Organism::Direction::LEWO);
					break;
				}
				else if (input == RIGHT) {
					czlowiek->set_direction(Organism::Direction::PRAWO);
					break;
				}
			}
			else {
		...
		if (is_player_alive && input == SPACE /*|| input == ABILITY2*/) {
			Human::Skill* tmpUmiejetnosc = czlowiek->get_skill();
		...
		if (input ==  ESC/* || input == EXIT2*/) {
			czyJestKoniecGry = true; //WYJSCIE Z GRY
			break;
		}
...
Organism* World::GetCzlowiek()
{
	return czlowiek;
}

Point World::WylosujWolnePole()
{
	for (int i = 0; i < sizeY; i++) {
		for (int j = 0; j < sizeX; j++) {
